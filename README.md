...
乡声集盒是“乡声万语”面向普通用户的地方语言工具与共建平台。

我们希望它首先是一个**真的有人愿意用的地方语言产品**：查一个家乡词、听真人怎么说、录下自己或家人的说法、比较不同地方的表达。与此同时，这些自然发生的使用与贡献，也能逐渐沉淀为有来源、有地域背景、可以继续整理的语言资料。

当前重点试点是莆仙方言，“兴化语记”是面向用户的旗舰地方入口；核心产品与数据模型则从一开始就面向多种方言、地方语言与民族语言扩展。

> **会写代码、懂产品、会说家乡话、手里有地方资料，都可以成为贡献者。**

如果你第一次来到这里，不需要先读懂全部领域模型。可以先从 [Contributor Onboarding](docs/CONTRIBUTOR_ONBOARDING.md) 开始，或者直接在 Issues 里告诉我们你想解决的问题。
...
贡献不只意味着写代码。前后端、测试、部署、数据导入、搜索体验、方言核对、资料来源整理、真实用户反馈和文档改进都很重要。

如果你还不熟悉 Git / GitHub，可以先去 [E-Dialect Git / GitHub 协作练习场](https://github.com/e-dialect/code-contributing-practice) 完成一次完整的 Fork → commit → Pull Request 练习。
